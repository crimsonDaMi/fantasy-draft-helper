...
| 2 | Position-based filtering | S | High | Position data already flows through draft-pick metadata. Likely just a query param on the recommendations endpoint + a filter control in the UI. |
...
1. **#2 Position-based filtering** — smallest effort, immediate draft-day value.
2. **#1 Dev/prod UI switch** — biggest experience upgrade for actual draft use.
   ...
   from the start, not retrofitted onto the dev one.
