...
### Phase 3 — only if the league wants a shared always-on tool — **complete**

7. ~~**#6 Clean user data separation**~~ — implemented.
8. ~~**#5 Multi-user support**~~ — implemented.

All candidate features from this document are now shipped. No new version
tag has been cut for Phase 2/3's work yet — per earlier discussion, that
happens once Phase 3 as a whole is considered done, which it now is. Next
step: cut a release, then decide what (if anything) goes on a future
